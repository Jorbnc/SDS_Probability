...
\begin{itemize}
	...
	\item Given that $B$ occured, we can update our model: Analyze possible causes or most likely scenarios for $B$. \\ \vspace{0.5mm}
			$\Red{B} \xrightarrow[P(\Blue{A_{i}}|\Red{B})]{\text{inference}} \Blue{A_i}$ \vspace{0.5mm}
	\item In other words, we use inference to analyze how likely is a scenario $A_{i}$, given that $B$ occured.
\end{itemize}
\vspace{0.5mm}
\begin{minipage}{0.34\linewidth}
	\centering
...
\sectionbox{\subsubsection{Independence of a collection of events}\topline
{\centering\fbox{%
...
\end{minipage}}\\}%
\vspace{1.5mm}
For instance, let $n=3$: \\\vspace{1mm}
\[
	\begin{rcases*}
	P(A_1 \cap A_2)=P(A_1)P(A_2) \\
	P(A_1 \cap A_3)=P(A_1)P(A_3) \\
	P(A_2 \cap A_3)=P(A_2)P(A_3) \\
	\end{rcases*} \text{Pairwise Independence}
\]
But we'll also need:\\
{\centering
	$P(A_1 \cap A_2 \cap A_3)=P(A_1)P(A_2)P(A_3)$
\\}\vspace{1mm}
Since it doesn't follow from the Pairwise Independence condition.
}

\sectionbox{\subsubsection{Independence vs. Pairwise Independence}\topline
Foo
}

\sectionbox{\subsubsection{Reliability}\topline
Foo
}
